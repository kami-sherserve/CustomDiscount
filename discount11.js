...
// ----- Create the country <select> element -----
...
function insertSelectorIntoContainer(container, checkoutButton) {
    ...
    container.insertBefore(select, checkoutButton);
    return select;
}
...
function insertSelectorInModal() {
    ...
    // Find the checkout button inside the modal's form
    ...
    // Insert before the button inside the modal form (or inside its parent)
    const container = modalForm; // insert directly into the form
    if (container.querySelector('#gift-ship-country-select')) return null;
    const select = createCountrySelect();
