...
// ----- Inject CSS for vertical stacking and full width -----
...
// ----- Country selector creation (shared) -----
...
// ----- Wait for main cart button (improved) -----
function waitForMainCheckoutButton() {
    return new Promise((resolve) => {
        const check = () => {
            const btn = document.querySelector('.cart__ctas .cart__checkout-button, button[name="checkout"]');
            if (btn && btn.closest('.cart__ctas')) {
                resolve(btn);
                return;
            }
...
async function insertCountrySelectorInCart() {
    ...
    const checkoutBtn = await waitForMainCheckoutButton();
    if (!checkoutBtn) return null;
    const parent = checkoutBtn.parentNode; // should be .cart__ctas
    const select = createCountrySelect('gift-ship-country-select');
    parent.insertBefore(select, checkoutBtn);
    return select;
}

// ----- Wait for drawer button -----
function waitForDrawerButton() {
    return new Promise((resolve) => {
        const check = () => {
            const drawer = document.getElementById('cart-notification');
            if (drawer) {
                const btn = drawer.querySelector('button[name="checkout"]');
                if (btn) {
                    resolve(btn);
                    return;
                }
...
async function insertCountrySelectorInDrawer() {
    const drawerBtn = await waitForDrawerButton();
    if (!drawerBtn) return null;
    if (document.getElementById('gift-ship-country-select-drawer')) return null;

    let container = drawerBtn.parentNode;
    while (container && !container.classList?.contains('cart-notification__links')) {
        container = container.parentNode;
    }
    if (!container) container = drawerBtn.parentNode;

    const select = createCountrySelect('gift-ship-country-select-drawer');
    container.insertBefore(select, drawerBtn);
    return select;
}

// ----- Watch for drawer appearing (alternative to waitForDrawerButton) -----
function observeDrawer(callback) {
            ...
            if (mutation.type === 'childList') {
                for (let node of mutation.addedNodes) {
                    if (node.nodeType === 1 && node.id === 'cart-notification') {
                        setTimeout(async () => {
                            const select = await insertCountrySelectorInDrawer();
                            if (select && callback) callback(select);
                        }, 100);
                        break;
                    }
                }
...
async function handleCountrySelection(selectedCode, shopOrigin) {
    ...
    // Also enable drawer button if exists
...
(async () => {
    ...
    // Insert selector in main cart (waits for button)
    const mainSelect = await insertCountrySelectorInCart();
    ...
    const changeHandler = async (e) => {
        ...
        // Sync other selectors (drawer) to the same value
    ...
    if (mainSelect) {
        ...
        console.warn('Main cart selector could not be inserted');
    }

    // Handle drawer: insert selector when it appears and bind handler
    observeDrawer(async (drawerSelect) => {
        drawerSelect.addEventListener('change', changeHandler);
        // If a country was already selected in main cart, sync to drawer
        const mainSelected = mainSelect ? mainSelect.value : null;
        if (mainSelected && mainSelected !== '') {
            drawerSelect.value = mainSelected;
            // Trigger change to enable drawer button if needed
            if (!scriptReady) drawerSelect.dispatchEvent(new Event('change'));
        } else {
    ...
    // Also check if drawer already exists (e.g., on cart page after page load)
    const existingDrawerSelect = await insertCountrySelectorInDrawer();
    if (existingDrawerSelect) {
        existingDrawerSelect.addEventListener('change', changeHandler);
        if (mainSelect && mainSelect.value && mainSelect.value !== '') {
            existingDrawerSelect.value = mainSelect.value;
        }
    }
