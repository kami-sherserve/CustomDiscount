...
async function insertCountrySelectorInCart() {
    ...
    // Find container (prefer .cart__ctas, then .cart__footer, then first button's parent)
    let targetContainer = null;
    for (let btn of checkoutElements) {
        const parent = btn.parentNode;
        if (parent && parent.classList?.contains('cart__ctas')) {
    ...
    if (!targetContainer) {
        ...
        if (cartFooter) targetContainer = cartFooter;
        else targetContainer = checkoutElements[0].parentNode;
    }
...
// ----- Insert in cart notification drawer (popup) -----
async function insertCountrySelectorInDrawer() {
    ...
    const drawerBtn = drawer.querySelector('button[name="checkout"]');
    if (!drawerBtn) return null;

    // Find the container .cart-notification__links
            ...
            if (mutation.type === 'childList') {
                for (let node of mutation.addedNodes) {
                    if (node.nodeType === 1 && node.id === 'cart-notification') {
                        // Drawer appeared – insert selector after a short delay
                        setTimeout(async () => {
...
(async () => {
    ...
    // Handle drawer: insert selector when it appears and bind same handler
    observeDrawer(async (drawerSelect) => {
        ...
        if (mainSelected && mainSelected !== '') {
            ...
            drawerSelect.dispatchEvent(new Event('change'));
        } else {
