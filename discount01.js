...
function insertCountrySelector() {
    ...
    // Prefer to insert into the direct parent of the first checkout button
    // because that parent is often a dedicated container (e.g., .cart__ctas)
    let targetContainer = checkoutButtons[0].parentNode;

    // If that parent is too generic (e.g., body), fall back to a known cart footer container
...
(async () => {
    ...
    // Original 500ms delay – preserved
    await new Promise(resolve => setTimeout(resolve, 500));

    // Insert country selector (only on cart page, directly above checkout button)
