...
// ----- CSS for vertical stacking on desktop -----
...
// ----- Country selector creation (can be used for both) -----
...
// ----- Main cart: wait for button and insert selector -----
async function waitForMainButton() {
...
async function insertDrawerSelector() {
    if (document.getElementById('gift-ship-country-select-drawer')) return null;
    const drawerBtn = await waitForDrawer();
...
// Also observe for drawer appearing dynamically (e.g., after "Add to cart" on product page)
            ...
            if (mutation.type === 'childList') {
                for (let node of mutation.addedNodes) {
                    if (node.nodeType === 1 && node.id === 'cart-notification') {
                        setTimeout(async () => {
                            const select = await insertDrawerSelector();
...
async function handleCountrySelection(selectedCode, shopOrigin) {
    ...
    // Also enable drawer's button if present
...
(async () => {
    ...
    // Insert main cart selector (waits for button)
    const mainSelect = await insertMainSelector();
    ...
    if (mainSelect) {
        mainSelect.addEventListener('change', onCountryChange);
    } else {
    ...
    const existingDrawerSelect = await insertDrawerSelector();
    if (existingDrawerSelect) {
        existingDrawerSelect.addEventListener('change', onCountryChange);
        if (mainSelect && mainSelect.value && mainSelect.value !== '') {
