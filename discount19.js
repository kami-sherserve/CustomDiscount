...
// ----- Inject CSS (responsive + disabled button styling) -----
...
// ----- Enable / disable all checkout elements -----
...
// ----- Intercept checkout clicks (international redirect) -----
...
// ----- Fix empty button in cart notification (Dawn theme quirk) -----
...
// ----- Create the country selector with label (15px, left align) -----
...
// ----- Insert selector into any container, before checkout button -----
...
// ----- Insert selector on cart page (works for Dawn & your theme) -----
function insertSelectorOnCartPage() {
    ...
    // Fallback: scan .cart__footer for any checkout element
    if (!checkoutButton) {
...
// ----- Sync all country selectors to the same value -----
...
// ----- Get shop origin (billing country) from your API -----
...
// ----- Handle country selection: set international flag and enable buttons -----
...
// ----- Event listener for country selectors (with sessionStorage) -----
...
// ----- Watch for modal appearance and insert selector there -----
function watchForModal(shopOrigin) {
    const observer = new MutationObserver((mutations, obs) => {
        ...
        if (modal && !modal.querySelector('#gift-ship-country-select')) {
            ...
            if (modalSelect) {
                // If we are inserting into the modal, we must disable checkout buttons
                // (the global scriptReady is false until a country is chosen)
                disableCheckoutElements(true, 'Select a country above');
                scriptReady = false;
...
(async () => {
    ...
    // Try to insert selector on the cart page
    const cartPageSelect = insertSelectorOnCartPage();
    const shopOrigin = await getShopOrigin();

    if (cartPageSelect) {
        // Insertion succeeded → disable checkout until a country is selected
        disableCheckoutElements(true, 'Select a country above');
        scriptReady = false;

        // Restore previously selected country from sessionStorage
        ...
        // Ensure checkout buttons are enabled (never disabled)
        disableCheckoutElements(false);
    }

    // Watch for modal (will insert selector and disable buttons there)
    watchForModal(shopOrigin);

    // Observe for any new checkout buttons that appear dynamically
    observeButtons();
