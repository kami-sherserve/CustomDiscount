...
// ----- Original isInternationalOrder (kept commented) -----
...
// ----- Country selector (inserted directly above checkout button) -----
...
function insertCountrySelector() {
    ...
    const checkoutButtons = getCheckoutElements();
    if (checkoutButtons.length === 0) return null;

    let targetContainer = null;

    // 1) Try to find the direct parent container of any checkout button,
    //    but prefer a container with class 'cart__ctas' or similar.
    for (let btn of checkoutButtons) {
        const parent = btn.parentNode;
        if (parent && (
            parent.classList?.contains('cart__ctas') ||
            parent.classList?.contains('cart__checkout') ||
            parent.className?.includes('ctas')
        )) {
    ...
    // 2) If not found, fall back to the first button's parent
    if (!targetContainer) {
        targetContainer = checkoutButtons[0].parentNode;
    }

    // 3) If that parent is still the body, try the cart footer
    if (targetContainer === document.body || targetContainer.tagName === 'BODY') {
    ...
    // Insert the select element right before the first checkout button inside the target container
    targetContainer.insertBefore(select, checkoutButtons[0]);
    return select;
}
